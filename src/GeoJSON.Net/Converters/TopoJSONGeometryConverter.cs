    ...
    public class TopoJSONGeometryConverter : JsonConverter
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            foreach (var geometryObject in objects) {
                String name = geometryObject.Key;
                JObject geometry = geometryObject.Value as JObject;
                String type = geometry["type"].Value<String>(); // TODO: Check for upper case here.

                switch (type.ToLowerInvariant())
                {
                    case "polygon":
                        TopoJSONPolygon poly = JsonConvert.DeserializeObject<TopoJSONPolygon>(geometry.ToString(), new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
                        geoJSONObjects.Add(poly);
                        break;
                    case "point":
                        Point p = JsonConvert.DeserializeObject<Point>(geometry.ToString(), new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
                        geoJSONObjects.Add(p);
                        break;
                    case "multipolygon":
                        MultiPolygon mpoly = JsonConvert.DeserializeObject<MultiPolygon>(geometry.ToString(), new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
                        geoJSONObjects.Add(mpoly);
                        break;
                    case "linestring":
                        LineString ls = JsonConvert.DeserializeObject<LineString>(geometry.ToString(), new JsonSerializerSettings { ContractResolver = new CamelCasePropertyNamesContractResolver() });
                        geoJSONObjects.Add(ls);
                        break;
                }
            }
